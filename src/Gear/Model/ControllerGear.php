...
class ControllerGear extends MakeGear
{
    ...
    public function getAddMethod($table)
    {
        ...
        $tableLine = $this->str('uline',$this->getFileName($this->str('class',$table)));
        $tableUrl = $this->str('url',$this->getFileName($this->str('class',$table)));
        $tableClass = $this->str('class',$this->getFileName($this->str('class',$table)));
    ...
    public function getEditMethod($module,$table)
    {
        ...
        $entity = $this->str('class',$table);
        $tableClass = $this->getFileName($this->str('class',$table));
        $tableVar = $this->str('var',$table);
        $tableLine = $this->str('uline',$this->getFileName($this->str('class',$table)));
        $tableUrl = $this->str('url',$this->getFileName($this->str('class',$table)));

        $aaa = $this->getIndent(1).trim('public function '.$this->getConfig()->getActionName('edit').'Action()').PHP_EOL;

        $aaa.= $this->getIndent(1).trim('{').PHP_EOL;

        $aaa.= $this->getIndent(2).trim('$this->session()->detachSession();').PHP_EOL;

        $aaa.= $this->getIndent(2).trim('    $id'.$entity.' = $this->getEvent()->getRouteMatch()->getParam(\'id\');').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('    if (!$id'.$entity.') {').PHP_EOL;
        $aaa.= $this->getIndent(3).trim('        return $this->redirect()->toRoute(\''.$this->str('url',$this->getModule()).'/'.$tableUrl.'/list\');').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('    }').PHP_EOL;

        $aaa.= $this->getIndent(2).trim('$'.$tableVar.'Logic = $this->getServiceLocator()->get(\'logic_'.$tableLine.'\');').PHP_EOL;
        ...
        $aaa.= $this->getIndent(3).trim('   if ($form->isValid()) {').PHP_EOL;
        $aaa.= $this->getIndent(4).trim('        $'.$tableVar.' = $'.$tableVar.'Logic->update($form->getData());').PHP_EOL;
        ...
        $aaa.= $this->getIndent(3).trim('$data = $'.$tableVar.'Logic->exportEntity($id'.$entity.');').PHP_EOL;
        $aaa.= $this->getIndent(3).trim('if ($data) {').PHP_EOL;
        $aaa.= $this->getIndent(4).trim('    $form->setData($data);').PHP_EOL;
        $aaa.= $this->getIndent(3).trim('} else {').PHP_EOL;
        $aaa.= $this->getIndent(4).trim(sprintf('    return $this->redirect()->toRoute(\'%s/%s/list\');',$this->str('url',$moduleClass),$this->str('url',$tableClass))).PHP_EOL;
        $aaa.= $this->getIndent(3).trim('}').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('}').PHP_EOL;
    ...
    public function getDelMethod($table)
    {
        ...
        $aaa = $this->getIndent(1).trim('public function '.$this->getConfig()->getActionName('del').'Action()').PHP_EOL;
        $aaa.= $this->getIndent(1).trim('{').PHP_EOL;

        $aaa.= $this->getIndent(2).trim('$'.$tableVar.'Logic = $this->getServiceLocator()->get(\'logic_'.$tableLine.'\');').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('$id'.$tableClass.' = $this->getEvent()->getRouteMatch()->getParam(\'id\');').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('if ($id'.$tableClass.') {').PHP_EOL;
        $aaa.= $this->getIndent(3).trim('    $'.$tableVar.'Logic->delete($id'.$tableClass.');').PHP_EOL;
        $aaa.= $this->getIndent(2).trim('}').PHP_EOL;
    ...
    public function getIndexMethod()
    {
        ...
        //var_dump($this->getConfig()->getTables());die();

        $tables = $this->getConfig()->getTables();
        ...
        foreach ($tables as $i => $v) {
            ...
            $view[***REMOVED*** =  '\''.$this->str('uline',$tableName).'\' => $c'.$this->str('var',$tableName).',';
