...
class AcceptanceTestServiceTest extends AbstractTestCase
{
    ...
    public function setUp()
    {
        ...
        $module = $this->getMockSingleClass('Gear\ValueObject\BasicModuleStructure', array('getModuleName', 'getTestAcceptanceFolder'));
        $module->expects($this->any())->method('getModuleName')->willReturn('SchemaModule');
        $module->expects($this->any())->method('getTestAcceptanceFolder')->willReturn(__DIR__.'/_files');
        $this->getAcceptanceTestService()->setModule($module);

        $phpRenderer = $this->mockPhpRenderer(__DIR__ . '/../../../../../view');
        $this->getAcceptanceTestService()->getTemplateService()->setRenderer($phpRenderer);
    }
